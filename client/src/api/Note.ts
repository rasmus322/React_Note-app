...
type FetchNoteListResponse = z.infer<typeof FetchNoteListScheme>

export function fetchNoteList(): Promise<FetchNoteListResponse> {
  return fetch("/api/notes").then((response) => response.json()).then((data) => FetchNoteListScheme.parse(data))
}
...
export const createNote = ({title, text}: { title: string, text: string}): Promise<void> => {
